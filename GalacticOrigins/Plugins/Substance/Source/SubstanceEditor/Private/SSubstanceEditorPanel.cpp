		...
		[
			...
			[
				...
				[
					...
					FSimpleDelegate::CreateSP(this, &SSubstanceEditorPanel::OnBrowseTexture, TextureObject))
				]
			]
			+SHorizontalBox::Slot()
			[
...
void SSubstanceEditorPanel::OnBrowseTexture(UObject* Texture)
{
	if (Texture)
	{
...
TSharedRef<SWidget> SSubstanceEditorPanel::GetInputWidgetSizePow2(TSharedPtr< input_inst_t > Input)
{
	...
	FString CurX = GetOutputSizeValue(Input, 0);
	FString CurY = GetOutputSizeValue(Input, 1);
...
{
	FScopedTransaction Transaction(TEXT("SubstanceEd"), NSLOCTEXT("SubstanceEditor", "SubstanceSetInput", "Substance set image input"), Graph);
	Graph->Modify();
