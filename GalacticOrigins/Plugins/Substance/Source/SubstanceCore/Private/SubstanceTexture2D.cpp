...
DEFINE_LOG_CATEGORY_STATIC(LogSubstanceTexture, Warning, All);
...
void USubstanceTexture2D::Serialize(FArchive& Ar)
{
	...
	if (bCooked && !this->IsDefaultSubobject())
	{
		...
		if (Ar.IsSaving())
		{
			...
			FirstMipToSerialize += FMath::Max(1, MipsToRegen);
		}
...
void USubstanceTexture2D::PostLoad()
{
	...
	{
		UE_LOG(LogSubstanceTexture, Error, TEXT("No parent instance found for this SubstanceTexture2D (%s). You need to delete the texture."),*GetFullName());
		Super::PostLoad();
		return;
	}
	...
	if (!Output)
	{
		...
		UE_LOG(LogSubstanceTexture, Error, TEXT("No matching output found for this SubstanceTexture2D (%s). You need to delete the texture and its parent instance."),*GetFullName());
		Super::PostLoad();
		return;
	}
	else
	{
		...
		{
			UE_LOG(LogSubstanceTexture, Error, TEXT("No matching output description found for this SubstanceTexture2D (%s). You need to delete the texture and its parent instance."),*GetFullName());
			Super::PostLoad();
			return;
		}
...
void USubstanceTexture2D::PostDuplicate(bool bDuplicateForPIE)
{
	...
	if (!bDuplicateForPIE)
	{
		...
		USubstanceGraphInstance* NewGraphInstance = Substance::Helpers::DuplicateGraphInstance(
			RefInstance,
	...
	if( Resource )
	{
			...
			FUpdateSubstanceTexture*,SubstanceData,SubstanceData,
		{
			...
			for( int32 MipIndex=0; MipIndex<SubstanceData->Owner->Mips.Num(); MipIndex++ )
			{
				...
				if (DestPitch == 0)
				{
					FMemory::Memcpy(TheMipData, MipMap.BulkData.Lock(LOCK_READ_ONLY), MipMap.BulkData.GetBulkDataSize());
				}
				else
				{
					...
					CopyTextureData2D(MipMap.BulkData.Lock(LOCK_READ_ONLY),TheMipData,MipMap.SizeY,PixelFormat,SrcPitch,DestPitch);
				}
...
void FSubstanceTexture2DDynamicResource::CreateSamplerStates(float MipMapBias)
{
	...
	FSamplerStateInitializerRHI SamplerStateInitializer
		(
		GSystemSettings.TextureLODSettings.GetSamplerFilter(SubstanceOwner),
...
void FSubstanceTexture2DDynamicResource::InitRHI()
{
	float MipMapBias =
		FMath::Clamp<float>(
			UTexture2D::GetGlobalMipMapLODBias() + ((SubstanceOwner->LODGroup == TEXTUREGROUP_UI) ? -SubstanceOwner->Mips.Num() : SubstanceOwner->LODBias), 
			0,
