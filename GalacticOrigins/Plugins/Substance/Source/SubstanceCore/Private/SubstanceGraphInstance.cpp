...
DEFINE_LOG_CATEGORY_STATIC(LogSubstanceGraphInstance, Log, All);
...
void USubstanceGraphInstance::BeginDestroy()
{
	...
	Substance::Helpers::UnregisterOutputAsImageInput(this);
...
void USubstanceGraphInstance::PostLoad()
{
	...
	{
		UE_LOG(LogSubstanceGraphInstance, Error, TEXT("Impossible to find parent package. You should delete the object."));	
		Super::PostLoad();
		return;
	}
	...
	if(!Instance)
	{
		UE_LOG(LogSubstanceGraphInstance, Error, TEXT("Error, no actual instance associated to the object. You should delete the object."));	
	...
	if (NULL == Instance->Desc)
	{
		UE_LOG(LogSubstanceGraphInstance, Error, TEXT("Error, Impossible to find parent desc, the package seems broken. You should delete the object."));
		Super::PostLoad();
		return;
	}
	...
	if (Instance->bHasPendingImageInputRendering || (Parent->GetGenerationMode() != ESubstanceGenerationMode::SGM_Baked && bCooked))
	{
